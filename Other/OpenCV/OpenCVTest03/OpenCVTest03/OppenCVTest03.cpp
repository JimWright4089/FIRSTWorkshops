...
int main( int argc, char** argv ) {
		...
		for(i = 1; i<3; ++i)
    {
			if((src[i-1]=cvLoadImage(argv[i], 1))== 0) 
      {
        ...
        //
        IplImage *hsv[NUMBER_OF_DIMS], *h_plane[NUMBER_OF_DIMS],*s_plane[NUMBER_OF_DIMS],*v_plane[NUMBER_OF_DIMS],*planes[NUMBER_OF_DIMS][NUMBER_OF_DIMS]; 
       	IplImage* hist_img[NUMBER_OF_DIMS];
		CvHistogram* hist[NUMBER_OF_DIMS];
...
//        	cvCopy( src[i], hsv[i]);
        	...
        	planes[i][0] = v_plane[i];
			planes[i][1] = h_plane[i];
...
//			cvCalcBackProject(planes[1],ftmp,hist[0]);
 		}
