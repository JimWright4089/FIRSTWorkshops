...
int main( int argc, char** argv ) 
{
  IplImage* src[2];
  IplImage* dst=0;
  IplImage* ftmp=0; //dst is what to display on
  int i;
  int type = 0;
  ...
  { 
    ...
    for(i = 1; i<3; ++i)
    {
      if((src[i-1]=cvLoadImage(argv[i]))== 0) 
      {
...
//      ftmp = cvCreateImage( cvGetSize(src[i]),8,4);
      cvZero(ftmp);
    }

    dst = cvCreateImage( cvGetSize(src[1]),8,1);
//    dst = cvCreateImage( cvSize(src[1]->width,src[1]->height),8,1);
//    dst = cvCreateImage( cvSize(900,900),8,1);
